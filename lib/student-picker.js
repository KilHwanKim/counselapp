...
/** 목록에 보이는 행 수 (스크롤 영역 높이 계산용) */
const VISIBLE_ROW_COUNT = 10;
/** 행 높이(px): py-2.5 + text-sm 기준 */
const ROW_HEIGHT_PX = 41;
const LIST_HEIGHT_PX = VISIBLE_ROW_COUNT * ROW_HEIGHT_PX + ROW_HEIGHT_PX; // 본문 10행 + 헤더 1행
...
function ensureModal() {
    const existing = document.getElementById('student-picker-modal');
...
export const StudentPicker = {
    ...
    close,
    /** 학생 선택 버튼 + 이름/ID 입력란 공통 바인딩 */
    bindButton(button, { nameEl, idEl, onSelect, clearPlaceholder } = {}) {
        if (!button) return;
        button.addEventListener('click', () => {
            StudentPicker.open((student) => {
                if (!student) return;
                if (idEl) idEl.value = student.id != null ? String(student.id) : '';
                if (nameEl) {
                    nameEl.value = student.name || '';
                    if (clearPlaceholder !== undefined) {
                        nameEl.placeholder = student.name ? '' : clearPlaceholder;
                    }
                }
